<template>
  ...
  <div class="box form-box p-5 my-3">
    <div class="mb-3 row">
      <div class="col-md-2">
        <label class="form-label">Which country are you currently living in?</label>
      </div>
      <div class="col-md-4">
        <input type="text" class="form-control" v-model="payload.country" />
      </div>
      <div class="col-md-2">
        <label class="form-label">Sex</label>
      </div>
      <div class="col-md-4 d-flex">
        <RadioButton :options="options" :Key="`gender`" @onChange="onRadioSelect" />
      </div>
    </div>
    <div class="mb-3 row">
      <div class="col-md-2">
    ...
    <div class="mb-3 row">
      ...
      <div class="col-md-4">
        <input type="date" class="form-control" v-model="payload.date_of_injury" />
      </div>
    </div>
    <div class="mb-3 row">
      <div class="col-md-2">
      ...
      <div class="col-md-4">
        <div class="col-md-4 d-flex">
          <RadioButton :options="knee" :Key="`knee`" @onChange="onRadioSelect" />
        </div>
      </div>
      <div class="col-md-2">
    ...
    <div class="mb-3 row">
      ...
      <div class="col-md-4">
        <input type="number" class="form-control" v-model="payload.injuries_same" />
      </div>
      <div class="col-md-2">
        <label class="form-label">Previous ACL injuries on other knee</label>
      </div>
      <div class="col-md-4">
        <input type="number" class="form-control" v-model="payload.injuries_other" />
      </div>
    </div>
    <div class="mb-3 row">
      <div class="col-md-2">
        <label class="form-label">Previous ACL reconstructions on same knee</label>
      </div>
      <div class="col-md-4">
        <input type="number" class="form-control" v-model="payload.reconstructions_same" />
      </div>
      <div class="col-md-2">
        <label class="form-label">Previous ACL reconstructions on other knee</label>
      </div>
      <div class="col-md-4">
        <input type="number" class="form-control" v-model="payload.reconstructions_other" />
      </div>
    </div>
    <div class="mb-3 row">
      <div class="col-md-2">
    ...
    <div class="mb-3 row">
      <div class="col-md-2">
        <label class="form-label">Other injuries sustained to knee (optional)</label>
      </div>
      <div class="col-10">
        <textarea type="text" class="form-control" rows="3" v-model="other_injuries" />
      </div>
    </div>
    <div class="d-flex justify-content-between" style="margin-top: 20px">
      <button class="btn btn-outline-secondary" @click="onCancel">Cancel</button>
      <div>
        <button class="btn btn-outline-secondary" @click="onSavetoDrafts">Save</button>
        <button class="btn btn-primary ml2" @click="onFinalize">Finalize</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions(useUserStore, ['getDoctorList', 'onSubmitDemographics']),
    ...
    onRadioSelect(key, value) {
      ...
      key === 'gender' ? (this.options = [...new_list]) : (this.knee = [...new_list])
    ...
    async onFinalize() {
      try {
        this.payload = {
          ...this.payload,
          survey_date: new Date(),
          patient: this.details.email,
          percentage: 100,
          draft: false,
          other_injuries: this.other_injuries
        }
        const res = await this.onSubmitDemographics(this.payload)
        if (res?.status === 200) {
          this.$router.push('/all-surveys')
        }
      } catch (error) {
  ...
  computed: {
    completionPercentage() {
      return Math.trunc(
        (Object.values(this.payload).filter((field) => field || field.length)?.length / 16) * 100
      )
    }
  },
  async mounted() {
